...
// jika user sudah login, maka jalankan perintah untuk insert, update, dan delete
        ...
        if (isset($_POST['simpan'])) {
            if (isset($_POST['kode_obat'])) {
                ...
                if ($query) {
                    ...
                    header("location: ../../main.php?module=penjualan&alert=2");
                }         
            }
        }
    }
    ...
    elseif ($_GET['act']=='delete') {
        if (isset($_GET['id'])) {
            $jumlah_terjual = $_GET['id'];
    
            // perintah query untuk menghapus data pada tabel obat
            $query = mysqli_query($mysqli, "DELETE FROM penjualan WHERE jumlah_terjual='$jumlah_terjual'")
                                            or die('Ada kesalahan pada query delete : '.mysqli_error($mysqli));

            // cek hasil query
            if ($query) {
                // jika berhasil tampilkan pesan berhasil delete data
                header("location: ../../main.php?module=penjualan&alert=3");
            }
        }
    }       
}       
